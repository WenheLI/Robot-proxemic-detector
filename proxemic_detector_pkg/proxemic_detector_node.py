    ...
    def depth_callback(self, msg): 
        ...
        try:
            ...
            if(self.DISPLAY and self.depth_image is not None):
                cv2.imshow("Depth Image", cv2.resize(self.depth_image, (self.rgb_image.shape[0], self.rgb_image.shape[1])))
                cv2.waitKey(1)
        except CvBridgeError as e:
            print(e)
    ...
    def detection_object_distance(self):
        ...
        for color in colors:
            ...
            for box in color_box:
                temp = self.extract_image_patch(resized_depth, box)
                img_patch_mean = np.mean(temp)
                if img_patch_mean < distance_to_object:
                    distance_to_object = img_patch_mean
                    selected_bbox = box

        if distance_to_object != 1e9 and len(selected_bbox) > 0:
